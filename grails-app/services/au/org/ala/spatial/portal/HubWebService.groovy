...
class HubWebService {
    ...
    def updateListQueries() {
        def threatened = getJson("${grailsApplication.config.lists.url}${grailsApplication.config.lists.threatenedSpeciesUrl}", false)
        def threatenedQ = "species_list_uid:(${threatened.lists.collect { it -> it.dataResourceUid }.join(" OR ")})"
        grailsApplication.config.threatenedQ = threatenedQ

        def invasive = getJson("${grailsApplication.config.lists.url}${grailsApplication.config.lists.invasiveSpeciesUrl}", false)
        def invasiveQ = "species_list_uid:(${invasive.lists.collect { it -> it.dataResourceUid }.join(" OR ")})"
    ...
    private URLConnection configureConnection(String url, boolean includeUserId, Integer timeout = null) {
        ...
        if (includeUserId) {
            ...
            if (user) {
                conn.setRequestProperty(grailsApplication.config.app.http.header.userId, user)
            }
        }
        conn
    }
    ...
    def getJson(String url, boolean includeUserId = true, Integer timeout = null) {
        def conn = null
        try {
    ...
    def doPostWithParams(String url, Map params) {
        ...
        try {
            ...
            def user = authService.getUserId()
            if (user) {
                conn.setRequestProperty(grailsApplication.config.app.http.header.userId, user) // used by ecodata
